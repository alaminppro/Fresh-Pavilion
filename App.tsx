...
interface SiteSettings {
  ...
  discord_webhooks: string; // JSON string of string[]
}
...
const App: React.FC = () => {
  ...
  const sendDiscordNotification = async (order: Order) => {
    try {
      const webhookUrls: string[] = JSON.parse(settings.discord_webhooks || '[]');
      if (webhookUrls.length === 0) return;

      const itemsList = order.items.map(i => `${i.name} (${i.quantity} ${i.unit})`).join(', ');
      
      const payload = {
        content: `🚨 **নতুন অর্ডার অ্যালার্ট!** 🛍️`,
        embeds: [{
          title: `অর্ডার আইডি: ${order.id}`,
          color: 3066993, // Green
          fields: [
            { name: "কাস্টমার", value: order.customerName, inline: true },
            { name: "ফোন", value: order.customerPhone, inline: true },
            { name: "লোকেশন", value: order.location, inline: true },
            { name: "মোট টাকা", value: `৳${order.totalPrice}`, inline: true },
            { name: "পণ্যসমূহ", value: itemsList }
          ],
          timestamp: new Date().toISOString(),
          footer: { text: "ফ্রেশ প্যাভিলিয়ন - আপনার ক্যাম্পাসের বিশ্বস্ত শপ" }
        }]
      };
      ...
      console.error("Error sending discord notifications:", err);
    }
  };
  ...
  const createOrder = async (orderData: Omit<Order, 'id' | 'created_at' | 'status'>): Promise<boolean> => {
    ...
    const newOrder: Order = { ...orderData, id: orderId, status: 'Pending', created_at: now };
