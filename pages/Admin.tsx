...
export const Admin: React.FC<AdminProps> = ({ 
  ...
  const toggleStock = (p: Product) => {
    onUpdateProduct({ ...p, stock: p.stock > 0 ? 0 : 10 });
  };
  ...
  const downloadCSV = () => {
    const headers = "Name,Phone,Order Count,Total Spent,Last Order\n";
    const rows = customers.map(c => 
      `"${c.name}","${c.phone}",${c.orderCount},${c.totalSpent},"${new Date(c.lastOrder).toLocaleDateString()}"`
    ).join("\n");
    const blob = new Blob([headers + rows], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `FP_Customers_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
        ...
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl w-full max-w-md border border-white/20">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              ...
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 outline-none focus:border-green-500 transition-all font-bold text-slate-800" placeholder="ইউজারনেম দিন" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Secret Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 outline-none focus:border-green-500 transition-all font-bold text-slate-800" placeholder="পাসওয়ার্ড দিন" />
            </div>
            <button type="submit" className="w-full py-5 rounded-2xl text-white font-black text-xl shadow-2xl shadow-green-500/30 bg-[#2E7D32] hover:bg-green-700 transition-all active:scale-[0.98]">প্রবেশ করুন</button>
          </form>
          <div className="mt-8 text-center">
            <button onClick={onBackToSite} className="text-slate-400 font-bold text-sm hover:text-green-600 transition-colors flex items-center justify-center gap-2 mx-auto">ওয়েবসাইটে ফিরে যান</button>
          </div>
        </div>
      </div>
    );
      ...
      <aside className="w-64 bg-white border-r border-slate-100 flex flex-col fixed h-full z-40 shadow-xl sidebar-no-scrollbar">
        ...
        <nav className="flex-grow px-4 py-6 space-y-1.5 overflow-y-auto sidebar-no-scrollbar">
          {(['Dashboard', 'Products', 'Orders', 'Customers', 'Settings'] as AdminTab[]).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold group ${activeTab === tab ? 'bg-green-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>
        ...
        <div className="p-4 border-t border-slate-50 bg-slate-50/20 space-y-2">
          <button onClick={onBackToSite} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-slate-900 text-white font-black text-xs transition-all hover:bg-slate-800 active:scale-95">🏠 ওয়েবসাইট হোম</button>
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-red-50 text-red-600 font-black text-xs border border-red-100 transition-all hover:bg-red-600 hover:text-white active:scale-95">🚪 লগ আউট</button>
        </div>
      </aside>
      ...
      <main className="flex-grow ml-64 p-10 h-screen overflow-y-auto scroll-smooth">
        <header className="mb-10 flex justify-between items-start">
          ...
          <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-50 text-right min-w-[160px]">
             ...
             <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{new Date().toLocaleDateString('bn-BD', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
          </div>
        </header>
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-50 hover:shadow-md transition-all">
                <div className={`w-10 h-10 ${storageUsage > 4 ? 'bg-red-50 text-red-500' : 'bg-purple-50 text-purple-500'} rounded-xl flex items-center justify-center text-2xl mb-4 shadow-inner`}>💾</div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
               ...
               <div className="bg-white p-8 rounded-[2.5rem] border border-slate-50 shadow-sm flex flex-col">
                  ...
                  <p className="text-slate-500 font-medium mb-6 leading-relaxed text-[10px]">Netlify তে ডেপ্লয় করার পর, ব্রাউজার স্টোরেজ লিমিট পার হয়ে গেলে আপনার নতুন পণ্য সেভ হবে না। সেক্ষেত্রে ডাটাবেজ ব্যবহার জরুরি।</p>
                  <div className="mt-auto pt-4 border-t border-slate-50">
                    <button onClick={handleResetProducts} className="w-full py-3.5 bg-red-500 text-white font-black rounded-xl shadow-md shadow-red-100 hover:bg-red-600 transition-all text-xs">ফ্যাক্টরি রিসেট করুন</button>
                  </div>
               </div>
            </div>
          </div>
        ...
        {activeTab === 'Products' && (
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-50">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
              ...
              <button onClick={() => handleOpenModal()} className="px-6 py-3 bg-green-600 text-white font-black rounded-xl flex items-center gap-2 shadow-md shadow-green-100 hover:-translate-y-1 transition-all text-sm"><span className="text-lg">+</span> পণ্য যোগ করুন</button>
            </div>
                    ...
                    <div className="flex justify-between items-end">
                       <div>
                          ...
                          <button 
                            ...
                            className={`mt-1 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider transition-all ${p.stock > 0 ? 'bg-green-100 text-green-600 border border-green-200' : 'bg-red-50 text-red-500 border border-red-100'}`}
                          >
                            {p.stock > 0 ? 'In Stock (স্টক আছে)' : 'Out of Stock (স্টক নেই)'}
                          </button>
                       </div>
                       <div className="flex gap-2">
                          <button onClick={() => handleOpenModal(p)} className="p-2 bg-blue-50 text-blue-500 rounded-lg hover:bg-blue-500 hover:text-white transition-all"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                          <button onClick={() => onDeleteProduct(p.id)} className="p-2 bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                       </div>
                    </div>
                 </div>
               ))}
                ...
                <tbody className="divide-y divide-slate-50">
                  {orders.map(o => (
                    <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-5"><span className={`px-2 py-0.5 rounded-md text-[7px] font-black uppercase tracking-widest ${o.status === 'Delivered' ? 'bg-green-100 text-green-700' : o.status === 'Cancelled' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>{o.status}</span></td>
                      <td className="py-5 text-right">
                        <select className="bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-[9px] font-black outline-none text-slate-800 focus:border-green-500 transition-all cursor-pointer" value={o.status} onChange={(e) => onUpdateOrderStatus(o.id, e.target.value as Order['status'])}>
        ...
        {activeTab === 'Customers' && (
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-50">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-black text-slate-800">গ্রাহক এনালিটিক্স</h2>
              <button 
                onClick={downloadCSV}
                className="px-5 py-2.5 bg-green-600 text-white font-black rounded-xl flex items-center gap-2 shadow-md hover:bg-green-700 transition-all text-xs"
              >
                📥 CSV ডাউনলোড করুন
              </button>
            </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-50">
               ...
               <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div className="flex items-end"><button onClick={handleAddUser} className="w-full py-3 bg-slate-900 text-white font-black rounded-xl hover:bg-slate-800 shadow-md active:scale-95 text-xs">জেনারেট করুন</button></div>
                  </div>
               </div>
               <div className="overflow-x-auto">
                 <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {subAdmins.map(admin => (
                        <tr key={admin.id} className="hover:bg-slate-50">
                          ...
                          <td className="py-5 text-right"><button onClick={() => setSubAdmins(subAdmins.filter(a => a.id !== admin.id))} className="px-3 py-1.5 bg-red-50 text-red-500 rounded-lg font-black text-[9px] hover:bg-red-500 hover:text-white">বন্ধ করুন</button></td>
                        </tr>
                      ))}
                    </tbody>
                 </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                ...
                <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold text-slate-800 outline-none focus:border-green-500 cursor-pointer text-sm" value={formState.category} onChange={e=>setFormState({...formState, category: e.target.value})}>
                  <option>খাবার</option><option>পানীয়</option><option>স্বাস্থ্য</option><option>অন্যান্য</option>
                </select>
              </div>
              <div className="space-y-1">
                ...
                <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold text-slate-800 outline-none focus:border-green-500 cursor-pointer text-sm" value={formState.unit} onChange={e=>setFormState({...formState, unit: e.target.value})}>
                  <option value="টি">টি (Piece)</option><option value="কেজি">কেজি (kg)</option><option value="গ্রাম">গ্রাম (gm)</option><option value="মিলি">মিলি (ml)</option><option value="লিটার">লিটার (L)</option>
                </select>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Initial Stock Status</label>
                <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold text-slate-800 outline-none focus:border-green-500 cursor-pointer text-sm" value={formState.stock > 0 ? 10 : 0} onChange={e=>setFormState({...formState, stock: Number(e.target.value)})}>
                  <option value={10}>In Stock (স্টক আছে)</option>
                  <option value={0}>Out of Stock (স্টক নেই)</option>
                </select>
              </div>
              <div className="col-span-2 space-y-2">
                ...
                <div className="flex items-center gap-5">
                  <div className="shrink-0 w-20 h-20 rounded-xl overflow-hidden border border-slate-100 bg-slate-50">{formState.image ? <img src={formState.image} className="w-full h-full object-cover" alt="preview" /> : <div className="w-full h-full flex items-center justify-center text-xl">📸</div>}</div>
                  <button onClick={() => fileInputRef.current?.click()} className="flex-grow py-6 border-2 border-dashed border-slate-100 rounded-xl font-black text-slate-300 hover:border-green-200 hover:text-green-400 transition-all text-sm">ছবি আপলোড করুন</button>
...
const StatCard = ({ label, val, icon, color }: { label: string; val: any; icon: string; color: string }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-50 hover:shadow-md transition-all">
      <div className={`w-10 h-10 ${colors[color]} rounded-xl flex items-center justify-center text-2xl mb-4 shadow-inner`}>{icon}</div>
